...
const millisecondsToSeconds = ms => Math.round(ms / 100);
...
const score = function (statusCurrent, scoreCurrent) {
    if (statusCurrent.isAlive) {
        ...
        return [
            `Lisa has been going for ${lifetimeString}(${millisecondsToSeconds(lifetime)}s).`,
            `The highscore is ${highscoreString}(${millisecondsToSeconds(highscore)}s)`
        ].join("\n");
    } else {
